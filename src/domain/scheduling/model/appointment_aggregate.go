...
const (
	...
	// AppointmentStatusCancelled is an appointment that has been cancelled within
	// policy. It is a terminal state that releases the slot the appointment held.
...
// the slot the appointment held and takes the appointment to a terminal state,
// applying any policy penalty the reason incurs; the same guards that gate a hold
// gate the cancel.
//
// The guards enforce, in order:
//
//   - Completeness: the appointment id and the cancellation reason must both be
//     present.
//   - Provider availability: the slot must fall within the provider's published
//     availability.
//   - Double-booking: a slot may be occupied by at most one appointment at a time.
...
func (a *AppointmentAggregate) cancelAppointment(cmd CancelAppointmentCmd) ([]shared.DomainEvent, error) {
	...
	if cmd.Reason == "" {
		return nil, ErrMissingCancelReason
	}
	...
	// Invariant: a slot may be booked by at most one appointment at a time.
	if a.SlotAlreadyBooked {
		return nil, ErrSlotDoubleBooked
	}
...
// releases the slot the appointment held and moves it into the terminal
// cancelled state, mirroring apply as the single mutation point for the cancel
// event during both command handling and replay.
