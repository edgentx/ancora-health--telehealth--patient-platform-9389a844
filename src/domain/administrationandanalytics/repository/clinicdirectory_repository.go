...
// ClinicDirectoryRepository persists and retrieves ClinicDirectoryAggregate
// instances. Concrete implementations (database-backed or in-memory) live
// outside the domain layer.
type ClinicDirectoryRepository interface {
	// Save persists the aggregate, honoring optimistic-concurrency semantics.
	Save(ctx context.Context, a *model.ClinicDirectoryAggregate) error
	// FindByID loads the aggregate identified by id.
	FindByID(ctx context.Context, id string) (*model.ClinicDirectoryAggregate, error)
}
