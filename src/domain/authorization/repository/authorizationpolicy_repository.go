...
// AuthorizationPolicyRepository persists and retrieves
// AuthorizationPolicyAggregate instances. Concrete implementations
// (database-backed or in-memory) live outside the domain layer.
type AuthorizationPolicyRepository interface {
	// Save persists the aggregate, honoring optimistic-concurrency semantics.
	Save(ctx context.Context, a *model.AuthorizationPolicyAggregate) error
	// FindByID loads the aggregate identified by id.
	FindByID(ctx context.Context, id string) (*model.AuthorizationPolicyAggregate, error)
}
